...
import { useGetProjectMilestoneDetailQuery, useEditMilestoneMutation } from '../services/epmApi';
import DateField from '../../../shared/components/DateField';

const EpmMilestoneEditScreen: React.FC<{ route: any }> = ({ route }) => {
  const { projectId, milestoneId } = route.params as { projectId: number; milestoneId: number };
  const navigation = useNavigation();
  const { colors, shadows } = useTheme();
  const toast = useToast();

  const { data, isLoading } = useGetProjectMilestoneDetailQuery({ projectId, milestoneId });
  ...
  useEffect(() => {
    if (milestone.milestoneName) {
      ...
      setNameAr(milestone.milestoneNameAr ?? '');
  ...
  const [save, { isLoading: saving }] = useEditMilestoneMutation();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Saved', 'Milestone updated.');
      navigation.goBack();
    } catch {
      toast.error('Failed', 'Could not save milestone.');
    }
  };
  ...
  return (
    <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={{ padding: 16, paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
      <View style={[styles.card, shadows.card, { backgroundColor: colors.card }]}>
        ...
        <TextInput style={[styles.input, { borderColor: colors.divider, color: colors.text, backgroundColor: colors.greyCard, height: 80 }]} value={description} onChangeText={setDescription} multiline placeholder="Optional description" placeholderTextColor={colors.textMuted} />
...
export default EpmMilestoneEditScreen;
