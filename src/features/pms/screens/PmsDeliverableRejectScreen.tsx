...
import { useRejectDeliverableMutation } from '../services/pmsApi';
import { usePmsRoles } from '../hooks/usePmsRoles';
import type { PmsWriteResponse } from '../types';

/**
 * Reject a PMS deliverable. Comments are MANDATORY.
 * Mirrors legacy SmartHelp/PMS/Controllers/DeliverablesController.Reject.
 * Backed by Mobile.spMobile_PMS_Deliverable_Reject.
 * Role gate: PMSRole.Admin OR DeliverableAdmin OR DeliverableApprover.
 */
const PmsDeliverableRejectScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { colors, shadows } = useTheme();
  const deliverableId: number = route?.params?.deliverableId;
  const name: string = route?.params?.name ?? '';

  const { canApproveDeliverable } = usePmsRoles();

  const [comments, setComments] = useState('');
  const [submitErr, setSubmitErr] = useState<string | null>(null);
  const [rejectDeliv, rejectState] = useRejectDeliverableMutation();
  ...
  const onSubmit = async () => {
    ...
    if (!trimmed) {
      setSubmitErr(t('pms.rejectionReasonRequired', 'Please enter a rejection reason'));
      return;
    }
    try {
      const resp = await rejectDeliv({ deliverableId, comments: trimmed }).unwrap();
      const env = peelEnvelope(resp);
      if (env?.success === false) {
        setSubmitErr(env.message || t('pms.actionFailed', 'Action failed'));
        return;
      }
      Alert.alert(t('pms.success', 'Success'), env?.message || t('pms.deliverableRejected', 'Deliverable rejected'));
      navigation.goBack();
    } catch (e: any) {
  ...
  const isBusy = rejectState.isLoading;
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={[styles.hero, shadows.card, { backgroundColor: colors.danger }]}>
          <Text style={styles.heroLabel}>{t('pms.rejectDeliverable', 'Reject Deliverable')}</Text>
          <Text style={styles.heroName} numberOfLines={3}>{name || `#${deliverableId}`}</Text>
        </View>
        ...
        <View style={[styles.card, shadows.card, { backgroundColor: colors.card }]}>
          <Text style={[styles.bodyText, { color: colors.textSecondary }]}>
            {canApproveDeliverable
              ? t(
                  'pms.rejectDeliverableHint',
                  'Provide a clear reason. The deliverable will be flagged as rejected and the assigned user will need to address the comments.',
                )
              : t(
                  'pms.rejectDeliverableNoRole',
                  'You do not have the role required to reject deliverables (Admin / Deliverable Admin / Deliverable Approver).',
                )}
          </Text>
        </View>
        ...
        <View style={[styles.card, shadows.card, { backgroundColor: colors.card }]}>
          <Text style={[styles.label, { color: colors.text }]}>
            {t('pms.rejectionReason', 'Rejection reason')} <Text style={{ color: colors.danger }}>*</Text>
          </Text>
          <TextInput
            value={comments}
            onChangeText={setComments}
            placeholder={t('pms.rejectionReasonPlaceholder', 'Explain what needs to change for the deliverable to be acceptable')}
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            onPress={onSubmit}
            disabled={isBusy || !canApproveDeliverable}
            style={[
              styles.btn,
              {
                backgroundColor: canApproveDeliverable ? colors.danger : colors.divider,
...
const styles = StyleSheet.create({
  ...
  input: {
    minHeight: 130, borderRadius: 8, borderWidth: StyleSheet.hairlineWidth,
...
export default PmsDeliverableRejectScreen;
