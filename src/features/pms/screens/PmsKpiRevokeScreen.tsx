...
import { useRevokeKPIMutation } from '../services/pmsApi';
import { usePmsRoles } from '../hooks/usePmsRoles';
import type { PmsWriteResponse } from '../types';

/**
 * Revoke a previously approved KPI definition.
 * Mirrors legacy SmartHelp/PMS/Views/KPIs/Details.cshtml — kpiRevokeApproval
 * form. Backed by Mobile.spMobile_PMS_KPI_Revoke (Admin or RevokeApprovals).
 */
const PmsKpiRevokeScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  ...
  const [revokeKpi, revokeState] = useRevokeKPIMutation();
  ...
  const onSubmit = async () => {
    setSubmitErr(null);
    try {
      const resp = await revokeKpi({ kpiId, comments: comments.trim() || undefined }).unwrap();
      const env = peelEnvelope(resp);
      if (env?.success === false) {
        setSubmitErr(env.message || t('pms.actionFailed', 'Action failed'));
        return;
      }
      Alert.alert(t('pms.success', 'Success'), env?.message || t('pms.kpiRevoked', 'Approval revoked'));
      navigation.goBack();
    } catch (e: any) {
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={[styles.hero, shadows.card, { backgroundColor: colors.warning }]}>
          <Text style={styles.heroLabel}>{t('pms.revokeApproval', 'Revoke Approval')}</Text>
        ...
        <View style={[styles.card, shadows.card, { backgroundColor: colors.card }]}>
          <Text style={[styles.bodyText, { color: colors.textSecondary }]}>
            {canRevokeApproval
              ? t(
                  'pms.revokeKpiHint',
                  'Revoking will clear the Approved date and ApprovedBy user on this KPI definition. The action is recorded in the KPI approval history.',
                )
              : t(
                  'pms.revokeKpiNoRole',
...
export default PmsKpiRevokeScreen;
