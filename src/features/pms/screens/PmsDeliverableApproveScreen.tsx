...
import { useApproveDeliverableMutation } from '../services/pmsApi';
import { usePmsRoles } from '../hooks/usePmsRoles';
import type { PmsWriteResponse } from '../types';

/**
 * Approve a PMS deliverable.
 * Mirrors legacy SmartHelp/PMS/Controllers/DeliverablesController.Approve.
 * Backed by Mobile.spMobile_PMS_Deliverable_Approve.
 * Role gate: PMSRole.Admin OR DeliverableAdmin OR DeliverableApprover.
 */
const PmsDeliverableApproveScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  ...
  const [approveDeliv, approveState] = useApproveDeliverableMutation();
  ...
  const onSubmit = async () => {
    setSubmitErr(null);
    try {
      const resp = await approveDeliv({ deliverableId, comments: comments.trim() || undefined }).unwrap();
      const env = peelEnvelope(resp);
      if (env?.success === false) {
        setSubmitErr(env.message || t('pms.actionFailed', 'Action failed'));
        return;
      }
      Alert.alert(t('pms.success', 'Success'), env?.message || t('pms.deliverableApproved', 'Deliverable approved'));
      navigation.goBack();
    } catch (e: any) {
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={[styles.hero, shadows.card, { backgroundColor: colors.success }]}>
          <Text style={styles.heroLabel}>{t('pms.approveDeliverable', 'Approve Deliverable')}</Text>
        ...
        <View style={[styles.card, shadows.card, { backgroundColor: colors.card }]}>
          <Text style={[styles.bodyText, { color: colors.textSecondary }]}>
            {canApproveDeliverable
              ? t(
                  'pms.approveDeliverableHint',
                  'Approving will lock the deliverable and mark it complete in the audit log.',
                )
              : t(
                  'pms.approveDeliverableNoRole',
                  'You do not have the role required to approve deliverables (Admin / Deliverable Admin / Deliverable Approver).',
                )}
          </Text>
        </View>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            ...
            style={[
              styles.btn,
              {
                backgroundColor: canApproveDeliverable ? colors.success : colors.divider,
...
export default PmsDeliverableApproveScreen;
