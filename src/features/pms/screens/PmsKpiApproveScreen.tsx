...
import { useApproveKPIMutation } from '../services/pmsApi';
import { usePmsRoles } from '../hooks/usePmsRoles';
import type { PmsWriteResponse } from '../types';

/**
 * KPI definition approval screen.
 * Mirrors legacy SmartHelp/PMS/Views/KPIs/Edit.cshtml when the user clicks the
 * "Approve" submit button. Backed by Mobile.spMobile_PMS_KPI_Approve which
 * gates the write to PMSRole.Admin server-side.
 */
const PmsKpiApproveScreen: React.FC<{ navigation: any; route: any }> = ({ navigation, route }) => {
  const { t } = useTranslation();
  const { colors, shadows } = useTheme();
  const kpiId: number = route?.params?.kpiId;
  const name: string = route?.params?.name ?? '';

  const { canApproveKpiDefinition } = usePmsRoles();

  const [comments, setComments] = useState('');
  const [submitErr, setSubmitErr] = useState<string | null>(null);
  const [approveKpi, approveState] = useApproveKPIMutation();
  ...
  const onSubmit = async () => {
    setSubmitErr(null);
    try {
      const resp = await approveKpi({ kpiId, comments: comments.trim() || undefined }).unwrap();
      const env = peelEnvelope(resp);
      if (env?.success === false) {
        setSubmitErr(env.message || t('pms.actionFailed', 'Action failed'));
        return;
      }
      Alert.alert(t('pms.success', 'Success'), env?.message || t('pms.kpiApproved', 'KPI approved'));
      navigation.goBack();
    } catch (e: any) {
  ...
  const titleHint = useMemo(
    () =>
      canApproveKpiDefinition
        ? t('pms.approveKpiHint', 'Confirm that this KPI is final and ready for use.')
        : t(
            'pms.approveKpiNoRole',
            'You do not have the PMS Admin role required to approve KPI definitions. The submit button has been disabled — the SP would also reject this request.',
          ),
    [canApproveKpiDefinition, t],
  );
    ...
    >
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={[styles.hero, shadows.card, { backgroundColor: colors.success }]}>
          <Text style={styles.heroLabel}>{t('pms.approveKpi', 'Approve KPI')}</Text>
          <Text style={styles.heroName} numberOfLines={3}>{name || `KPI #${kpiId}`}</Text>
        </View>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            onPress={onSubmit}
            disabled={isBusy || !canApproveKpiDefinition}
            style={[
              styles.btn,
              {
                backgroundColor: canApproveKpiDefinition ? colors.success : colors.divider,
...
const styles = StyleSheet.create({
  ...
  errorCard: {
    borderRadius: 10, padding: 12, borderWidth: StyleSheet.hairlineWidth,
  },
...
export default PmsKpiApproveScreen;
